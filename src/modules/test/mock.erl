...
replace_module(Key, {Module, MockModule}) when is_atom(MockModule) ->
    {ok, [Data]} = file:consult("./test.config"),
    {_, Dir} = lists:keyfind(Key, 1, Data),
    ModuleFileName = atom_to_list(MockModule) ++ ".erl",
    replace_module({Module, filename:join(Dir, ModuleFileName)}),
    ok.

replace_module ({Module, Mock_filename}) when is_list(Mock_filename) ->
    uninstall (Module),    
    {ok, _, Binary} = compile:file(Mock_filename, [binary]),

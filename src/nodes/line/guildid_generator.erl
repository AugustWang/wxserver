...
handle_call({gen_newid}, _From, 
	...
	NewCurIndx = case CurIndex of
		undefined -> idgen:get_idmax({guildid,ServerId}) + 1;
		_-> CurIndex+1
	end,
	Reply =  {ServerId,NewCurIndx},
	idgen:update_idmax({guildid,ServerId},NewCurIndx),
...
handle_info({init_index}, #state{serverid=ServerId}=State)->
	IdMax = idgen:get_idmax({guildid,ServerId}),

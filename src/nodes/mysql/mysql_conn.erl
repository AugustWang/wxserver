...
send_msg(Pid, Msg, From, Timeout) ->
    Self = self(),
    %slogger:msg("===============enter fetch ============== ===:~p-~p~n",[Pid, Msg]),
    Pid ! Msg,
    case From of
	Self ->
		...
		{fetch_result, Pid, Result} ->
 			%slogger:msg("===============fetch result ============== ===:~p-~p-~p~n",[fetch_result, Pid, Result]),
		    Result
	    after Timeout ->
...
%%     ?Log2(LogFun, debug, "fetch ~p (id ~p)", [Query1,RecvPid]),
    %slogger:msg("===============do query============== ===:~n"),
    Packet =  <<?MYSQL_QUERY_OP, Query1/binary>>,
    case do_send(Sock, Packet, 0, LogFun) of
	ok ->
	    Qr = get_query_response(LogFun,RecvPid,
				    Version),
    	%slogger:msg("===============query result============== ===:~p~n",[Qr]),        
        Qr;
	{error, Reason} ->

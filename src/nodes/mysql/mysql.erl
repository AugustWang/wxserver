...
%%%
%%% Copyright (c) 2001-2004 Kungliga Tekniska H锟絞skolan
...
init_pool()->
	...
	Args = [mysql_util:get_pool(),Host,Port,User,Pass,DBName,undefined,undefined],
	case gen_server:call(?SERVER, {init_pool,Args}) of
		ok->
			InitConnNums = env:get2(mysql,init_conn_nums,20),
			io:format("peopel init_pool success,Num=~p~n",[InitConnNums]),
			slogger:msg("peopel init_pool success,Num=~p~n",[InitConnNums]);
		{error,_}->
			nothing
	end.
...
fetch(PoolId, Query, Timeout) -> 
    case get(?STATE_VAR) of
	undefined ->
		slogger:msg("===============start call server ================="),        
		call_server({fetch, PoolId, Query}, Timeout);
%slogger:msg("===============end call server =================");
	State ->
	    mysql_conn:fetch_local(State, Query)
    end.
...
init([PoolId, Host, Port, User, Password, Database, LogFun, Encoding]) ->
	...
	Encoding1 = if Encoding == undefined -> ?DEFAULT_ENCODING; true -> Encoding end,
    InitConnNums = env:get2(mysql,init_conn_nums,20),
	InitConnFun = 
		fun(Index,StateAcc)->
			case mysql_conn:start_link(Host, Port, User, Password, Database, LogFun1,
				...
				{ok, ConnPid} ->
	    			Conn = new_conn(PoolId, ConnPid, true, Host, Port, User, Password,
			    					Database, Encoding1),
	    			add_conn(Conn, StateAcc);
				{error, Reason} ->
	    			?Log(LogFun1, error,
		 			"failed init MySQL NO."++integer_to_list(Index)++" connection handler, "
		 			"exiting"),
	    			{stop, {error, Reason}}
			end
		end,
	...
	{ok, ConnPid} ->
		...
		NewState2 = lists:foldl(InitConnFun, NewState, lists:seq(1, InitConnNums)),
	    {ok, NewState2};
...
handle_call({init_pool, Args}, _From, State) ->
    case init(Args) of
		{ok,ReturnState}->
